...
\begin{prop}[Universal property of the tensor product]  Suppose $V$ and $W$ are vector spaces over the field $K$.  
\end{prop}
...
Now, we must show that this spanning set is linearly independent.  Suppose that $\sum_{i,j} c_{ij} e_i \otimes f_j = 0$, where all but finitely many $c_{ij}$ are 0.  We want to show that $c_{ij} = 0$ for every $i \in I, j \in J$. Fix two elements $ i_0 \in I$ and $j_0 \in J$.  To show that $c_{i_0 j_0} = 0$, consider the bilinear map
\begin{align*}
V \times W &\to K \\
...
, and assume we have bases $\{ a_1, \ldots, a_n\}$ for $V$  and $\{b_1, \ldots, b_m \}$ for W.
