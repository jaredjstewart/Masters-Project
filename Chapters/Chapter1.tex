...
\begin{defn}
The \textbf{character} of a representation $\varphi$ is the function $\chi_\varphi \colon G \to \mathbb{C}$ defined by $\chi_\varphi(g) = \text{Tr}(\varphi(g))$.
\end{defn}

\begin{prop}
\begin{enumerate}
\item $\chi$ is conjugation invariant: $\chi (h g h^{-1}) = \chi (g)$ for all $g , h \in G$.
\item
\end{enumerate}
\end{prop}
