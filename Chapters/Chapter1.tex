...
\end{align*}
Since both vector spaces have the same dimension, $\psi \circ \psi^{-1}$ must also be the identity map, and $\psi$ is an isomorphism of vector spaces.  Now suppose we have representations $\rho_V, \rho_W, \rho_U$ of $G$ on $V, W$ and $U$.  Then we claim $\psi$ is $G$-linear.  Recall that by definition, 
\[ \rho_{\text{Hom}(V, U \oplus W)}(g)(f) = \rho_{V \oplus W} (g) \circ f \circ \rho_V (g^{-1}).\]
