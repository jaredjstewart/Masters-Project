...
\begin{align*}		
 ...
 \end{align*}		
for all $g \in G$. Note that $\rho_{\text{Hom}(V,W)}(g)(f)$ is certainly a linear map $V \to W$ since the composition of linear maps is linear.
\end{defn}
...
\begin{enumerate}		
\item For all $g \in G$,  $\rho_{\text{Hom}(V,W)}(g)$ is invertible.		
...
\end{cor}
\begin{proof}
Let $0 \neq \phi \in \text{Hom}_G(V,W)$.  If $v \in \text{ker}(\phi)$, then $\phi(v) = 0$ implies that  $\phi (gv) = g  \phi(v) = g  0 =0$, i.e. $ gv \in \text{ker}(\phi)$. Similarly, if $v \in \text{im}(\phi)$, then $v = \phi(w)$ implies that $\phi (gw) = g \phi(w) = gv$, i.e. $gv \in \text{im}(\phi)$.  Thus $\text{ker}(\phi)$ and $\text{im}(\phi)$ are both $G$-invariant.  
