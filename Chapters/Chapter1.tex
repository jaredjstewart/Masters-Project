...
\end{align*}
Thus $\rho_{\text{Hom}(V,W)}$ is a representation of $G$.
\end{proof}
...
\end{defn}
Note that $V^G$ is a subspace of $V$, and is clearly also a subrepresentation.   It is isomorphic to a trivial representation of some dimension.
...
\end{align*}
which is exactly the condition that $f$ is $G$-linear, i.e. $f \in \text{Hom}_G (V,W)$.
\end{proof}
