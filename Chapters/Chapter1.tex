...
\begin{cor}
If $V$ and $W$ are irreducible, the space $\text{Hom}^G(V,W)$ is $1$-dimensional if the representations are isomorphic, and ${0}$ otherwise.  In the first case, any non-zero map is an isomorphism.
\end{cor}
\begin{proof}
...
Let $\psi$ be another interwining operator from $V$ to $W$.  Then $\phi ^{-1} \circ \psi$ is an intertwining operator from $V$ to $V$.  We apply Schur's Lemma over $\mathbb{C}$ to see that $\phi ^{-1} \circ \psi = \lambda \text{Id}$, hence $\psi = \lambda \phi$.
\end{proof}
